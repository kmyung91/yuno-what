...
document.querySelectorAll("a").forEach(anchor => {
    anchor.addEventListener("contextmenu", e => {
        ...
        // Add data-link attribute to each menu item with the href of the clicked link
        contextMenuItems.forEach(item => {
            item.dataset.link = clickedAnchor.href;
        });
    });
});

// Create a function to update the height
function updateHeight() {
  var windowH = window.innerHeight;

  document.querySelectorAll('section .container-interactivebg').forEach(function(container) {
    var curr = container;
    var parent = curr.parentElement;
    if (!parent.classList.contains('bottom')) {
      curr.style.height = windowH + 'px';
    }
  });
}
...
$(document).ready(function() {
    // Add 'down' class to all 'li' and 'span' elements
    $('.numbered-list li, .pagination span').addClass('down');

    // Remove 'down' class from the first 'li' and 'span' elements and add the 'active' class
    $('.numbered-list li:first, .pagination span:first').removeClass('down').addClass('active');

    // Your activateBgr function
    function activateBgr() {
      subtleMultipleSplats(parseInt(Math.random() * 20) + 5)
    }

    $('.numbered-list li').click(function() {
        // Get the clicked li and its index
        var clickedLi = $(this);
        var clickedIndex = clickedLi.index();

        // Update classes on li and span elements
        $('.numbered-list li').removeClass('active').removeClass('up').addClass('down');
        $('.pagination span').removeClass('active').removeClass('up').addClass('down');
        clickedLi.removeClass('down').addClass('active');
        $('.pagination span').eq(clickedIndex).removeClass('down').addClass('active');
        if(clickedIndex > 0) {
          $('.numbered-list li').eq(clickedIndex-1).removeClass('down').addClass('up');
          $('.pagination span').eq(clickedIndex-1).removeClass('down').addClass('up');
        }
        if(clickedIndex < $('.pagination span').length - 1) {
          $('.numbered-list li').eq(clickedIndex+1).removeClass('up').addClass('down');
          $('.pagination span').eq(clickedIndex+1).removeClass('up').addClass('down');
        }

        // Call activateBgr function
        activateBgr();
    });
});
...
if (!$('html').hasClass('ie11')) {
    ...
    var sunrays;
    var sunraysTemp;
    var ditheringTexture = createTextureAsync('https://digi-globe.com/wp-content/uploads/2023/05/dither.png');
    ...
    function generateColor() {
        var h = Math.random() > 0.5 ? Math.random() * 0.2 : 0.6 + Math.random() * 0.4;  // generate hues around red and blue
        var c = HSVtoRGB(h, 1.0, 0.5);
        c.r *= 0.10;
        c.g *= 0.10;
        c.b *= 0.10;
        return c;
...
function inactivityTime() {
    // Define cf as an object if not already defined, to store the interval ID
    window.cf = window.cf || {};
    ...
    function resetTimer() {
        ...
        // Clear the stopInterval if it is already set
        if (cf.stopInterval) {
            clearTimeout(cf.stopInterval);
        }

        // Set an interval to stop the inactivityInterval after 2 minutes
        cf.stopInterval = setTimeout(function() {
            killTimer();
